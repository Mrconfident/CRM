...
<div class="LoginBusiness">
  ...
  <div class="container">
    <span class="psw">Login Agent...? <router-link to="/loginagent"> Login </router-link></span>
  </div>
</div>
</template>
<script>
    ...
    authenticate () {
      ...
      axios.post(url, { username: this.username, password: this.password, client: 'crm' }).then((response) => {
        ...
        if (response.data.success !== true) {
          ...
          setCookie('b_id', response.data.data.jwt, 10)
          console.log(response.data + '************************')
          const url = api.BASE_URL_LOCAL + 'crmorg/validateAdmin'
          const headers = {
